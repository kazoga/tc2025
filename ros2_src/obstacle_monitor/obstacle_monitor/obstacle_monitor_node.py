...
# ---- 差分・注意点 ----
# 1) メッセージ型: ユーザー指定に従い route_msgs/ObstacleAvoidanceHint を使用。
#    stamp は builtin_interfaces/Time と想定。定義差異がある場合はフィールド名に合わせて修正が必要。
# 2) フィルタ: legacy と同様に ±90° の前方のみを評価。x<=max_obstacle_distance_m の窓を導入。
# 3) オフセット算出: legacy の calcAvoidanceOffset をそのまま関数化し、左右独立に適用。
#    結果は 0.0 の場合を除き下限 min_offset_lower_bound_m (=0.75m) を適用。
# 4) viewer: legacy の pixel 座標系（x前方→下、y左→右）・ライン描画（ロボット幅線/最大距離線/回避ライン）を踏襲。
# 5) EMA/デバウンス等の近代化要素は削除（“踏襲”要件に合わせ簡素化）。
# 6) QoS: Pub は BestEffort/Volatile/Depth=1、Sub は SensorDataQoS。
