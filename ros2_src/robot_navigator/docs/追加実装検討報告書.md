...
## 4. 設計方針案
### 4.1 入力切替パラメータ
- 新パラメータ `obstacle_distance_source`（`scan` / `hint` の 2 値）を追加し、既定値を `hint` とする。`ParameterDescriptor` の `additional_constraints` に許容値を明記し、launch や YAML から切り替えられるようにする。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L62-L152】
...
- `obstacle_distance_source` に応じて購読をセットアップする。
  ...
  - `hint` モード: `ObstacleAvoidanceHint` を BEST_EFFORT / KEEP_LAST(depth=1) で購読し、`front_range` の有限値なら `obstacle_distance` を更新、無効値 (`inf` や `NaN`) の場合は `None` に戻す。
...
### 4.5 設定ファイル・依存関係
- `params/default.yaml` へ新パラメータを追加し、既定値を `hint` としておく。`scan` を利用したい運用では launch で `obstacle_distance_source:=scan` を明示する。【F:ros2_src/robot_navigator/params/default.yaml†L1-L22】
...
3. **境界条件**: `front_range=inf` や LaserScan 欠損などで確実に停止指令が出るかをチェック。
