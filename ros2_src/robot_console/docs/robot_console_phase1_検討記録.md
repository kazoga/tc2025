...
7. **制御コマンドパネル**
   ...
   - `manual_start` は送信ボタンと最終送信時刻のみを掲示し、`sig_recog` はGO／STOP／未定義のラジオボタンと送信ボタン・履歴行のみで構成する。
   - `obstacle_avoidance_hint` では `front_blocked` チェックボックスと余裕距離・左右オフセットのSpinboxを1段に集約し、固定値送出の開始／停止ボタンを並べて即時操作できるようにする（チェックボックスは廃止）。
   - `road_blocked` は送信ボタンと現在値／最終送信時刻を1行で示し、手動で切替が必要な時のみアクセスする想定とする。
