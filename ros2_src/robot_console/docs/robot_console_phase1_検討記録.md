...
### 4. ダッシュボード中心の表示・操作方針
1. **状態サマリ**
   - `route_state`・`follower_state`・`manager_status` のカードを横並びの固定比率で配置し、ウィンドウサイズが変わっても上端と幅が揃うよう uniform 設定を適用する。
   - `route_state.current_index` と `follower_state.current_index` を進捗バーと数値で比較し、ズレ検出時は警告色で強調する。
   - `manager_status.state`／`last_cause`／`last_transition` を専用カードにまとめ、再計画や異常要因を即時に参照できるようにする。
...
6. **ノード起動・設定パネル**
   ...
   - スクロール可能なフレーム構造とし、ウィンドウが狭い場合でも全パッケージ設定へアクセスできるようにする。

7. **制御コマンドパネル**
   - `manual_start` ボタン、`sig_recog` 値選択、`road_blocked` トグルを1列にまとめ、バナー直下のパネルから即座に操作できるようにする。
   - `obstacle_avoidance_hint` は固定値送出のオン／オフとともに、`front_blocked` の真偽・余裕距離・左右オフセットを Spinbox で入力できるようにし、有効化中は値変更を即座に反映する。

8. **コンソールログパネル（タブ切替）**
   - ダッシュボードとは別タブのログ画面を用意し、同一タブ内を縦方向に4分割して `route_manager`・`route_follower`・`obstacle_monitor`・`robot_navigator` の stdout/stderr を個別のスクロールパネルで閲覧する。
