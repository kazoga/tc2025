...
- **パラメータと初期化**: 最大速度・加速度、位置/姿勢許容誤差、レーザセンサの検出範囲、トピック名などを ROS パラメータとして宣言し、初期化時に取得して内部変数へ格納する。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L62-L116】
- **通信インタフェース**: `/cmd_vel` 出力用 Publisher と、進行方向可視化の `/direction_marker` Publisher を保持。入力は `/odom`・`/amcl_pose`・`/active_target`（パラメータで変更可）・`/scan` を購読して速度・現在姿勢・目標姿勢・障害物距離を更新する。制御周期は `control_rate_hz`（既定20Hz）でタイマー駆動される。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L131-L156】【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L177-L230】
- **障害物検出**: `/scan` からロボット前方矩形領域の最短距離を計算し、障害物距離として保持する。無効レンジのフィルタリングや矩形条件判定が実装されている。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L190-L216】
...
- **初期座標系の決定**: `/active_target` を初回受信するまでシミュレータの更新・配信処理を開始せず、1秒おきに待機ログを INFO で出力する。初回メッセージを受け取ると内部状態と map→odom 変換を同時に初期化し、その後 `/amcl_pose` を購読して原点の再取得にも対応する。【F:ros2_src/robot_navigator/robot_navigator/robot_simulator_node.py†L151-L223】
...
3. robot_navigator は `/amcl_pose` と `/active_target`（パラメータでトピック変更可能）を購読し、姿勢・目標が揃えば PID/障害物判定で `/cmd_vel` を出力する。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L148-L381】
...
2. **目標トピック名の初期設定差異**: robot_navigator の既定 `goal_topic` は `/active_target` として定義され、route_follower が配信する PoseStamped をそのまま受信できる。警告メッセージもパラメータ値を参照するよう更新されており、トピック変更時にログ表記との不整合が発生しない。既存環境で `/goal` など別トピックを利用する場合は、パラメータまたは launch で値を変更する必要がある。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L82-L261】【F:ros2_src/route_follower/route_follower/route_follower_node.py†L188-L211】
...
4. **`/amcl_pose` フィードバックの循環防止**: `/amcl_pose` を同じノードが publish/subscribe しているため、自ノードが送信した PoseWithCovarianceStamped を即座に受信し、原点更新ロジックが毎周期その値を map→odom 原点として積み増す挙動が発生した。直前に送信したメッセージのタイムスタンプを保持し、同じ値を持つ受信メッセージを無視することで外部ノードからの再初期化指示のみに反応するようガードした。【F:ros2_src/robot_navigator/robot_navigator/robot_simulator_node.py†L113-L117】【F:ros2_src/robot_navigator/robot_navigator/robot_simulator_node.py†L178-L184】【F:ros2_src/robot_navigator/robot_navigator/robot_simulator_node.py†L251-L272】
...
- **停止処理の挙動確認**: 入力欠損時の停止処理は `/amcl_pose` `/odom` `goal_topic` の各パラメータ名を参照する WARN ログとゼロ Twist の発行で構成される。これによりトピック名変更後もオペレータが不足入力を正しく把握でき、安全停止も維持されることを確認した。【F:ros2_src/robot_navigator/robot_navigator/robot_navigator_node.py†L144-L260】
